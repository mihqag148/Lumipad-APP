    ...
    private void UpdateProductSpecificText()
    {
            ...
            _screensaverAnimation is null)
        {
            ScreensaverMediaInfo.Text =
                IsPixelProActive
                    ? L(
                        "ILI9486 480×320 · PXQ quality cap up to 2 MiB · minimum 360×240 / RGB565 / 15 FPS · Delta + RLE · Windows-style layouts.",
                        "ILI9486 480×320 · PXQ tối đa 2 MiB · tối thiểu 360×240 / RGB565 / 15 FPS · Delta + RLE · kiểu hiển thị như Windows.")
                    : L(
    ...
    private async Task PrepareScreensaverMediaAsync()
    {
        ...
        try
        {
                ...
                if (pixel)
                {
                    ...
                    string emergency =
                        pixelPackedInfo.EmergencyFps
                            ? L(
                                " · emergency FPS used to keep the 2 MiB cap without dropping below RGB565 / 15 FPS / 360×240",
                                " · đã hạ FPS khẩn cấp để giữ giới hạn 2 MiB mà không hạ dưới RGB565 / 15 FPS / 360×240")
                            : "";

                    pixelGifSummary =
                        L(
                            $"PIXEL PXQ · {storedKb:0.#} KB (≤2 MiB quality cap) from {sourceKb:0.#} KB · logical 480×320 · stored {pixelPackedInfo.StorageWidth}×{pixelPackedInfo.StorageHeight} → 480×320 · {pixelPackedInfo.Fps} FPS · {pixelPackedInfo.ColorMode} · minimum RGB565 / 15 FPS / 360×240 · Delta + RLE · {pixelPackedInfo.ScaleMode}{emergency}",
                            $"PIXEL PXQ · {storedKb:0.#} KB (giới hạn chất lượng ≤2 MiB) từ {sourceKb:0.#} KB · logical 480×320 · lưu {pixelPackedInfo.StorageWidth}×{pixelPackedInfo.StorageHeight} → 480×320 · {pixelPackedInfo.Fps} FPS · {pixelPackedInfo.ColorMode} · tối thiểu RGB565 / 15 FPS / 360×240 · Delta + RLE · {pixelPackedInfo.ScaleMode}{emergency}");
                }
            ...
            ScreensaverSendStatus.Text =
                pixel
                    ? L(
                        "Ready. PIXEL GIF is packed as PXQ with Delta + RLE up to 2 MiB, minimum 360×240 / RGB565 / 15 FPS; images use JPEG quality 100. Display refresh remains 60 Hz.",
                        "Đã sẵn sàng. GIF PIXEL được đóng gói PXQ bằng Delta + RLE tối đa 2 MiB, tối thiểu 360×240 / RGB565 / 15 FPS; ảnh dùng JPEG quality 100. Màn hình vẫn làm tươi 60 Hz.")
                    : L(
